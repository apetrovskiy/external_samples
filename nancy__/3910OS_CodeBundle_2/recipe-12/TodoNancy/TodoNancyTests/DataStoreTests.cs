namespace TodoNancyTests
{
  ...
  public class DataStoreTests
  {
    ...
    {
      var allTodos = Enumerable.Range(0, expected).Select(i => new Todo { id = i, userName = User.Anonymous.UserName });
      var returnValue = (new Task<IEnumerable<Todo>>(() => allTodos));
      returnValue.Start();
      A.CallTo(() => fakeDataStore.GetAll()).Returns(returnValue);
